...
properties colorGraph name = describe name $ do
  prop "no adjacent vertexes have same color" $ \g ->
    colorGraph g `shouldBeVertexColoringFor` g
...
shouldBeVertexColoringFor cs g =
  counterexample ("coloring = " ++ show (sortOn fst cs)) $
  conjoin $ do
    e <- es
