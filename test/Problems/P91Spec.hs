...
knightsTourProperties knightsTour name = describe name $ do
  -- Board sizes known to have no knight's tours.
  describe "has no tours" $ do
    it "with size 2" $ knightsTour 2 (1,1) `shouldBe` Nothing
    it "with size 3" $ knightsTour 3 (1,1) `shouldBe` Nothing
    it "with size 4" $ knightsTour 4 (1,1) `shouldBe` Nothing

  -- For naive backtracking algorithms,
  -- even slightly larger sizes can take a very long time.
...
closedKnightsTourProperties closedKnightsTour name = describe name $ do
  -- Board sizes known to have no closed knight's tours.
  describe "has no tours" $ do
    it "with size 2" $ closedKnightsTour 2 `shouldBe` Nothing
    it "with size 3" $ closedKnightsTour 3 `shouldBe` Nothing
    it "with size 4" $ closedKnightsTour 4 `shouldBe` Nothing
    it "with size 5" $ closedKnightsTour 5 `shouldBe` Nothing
