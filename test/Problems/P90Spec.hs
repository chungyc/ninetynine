...
properties queens name = describe name $ do
  modifyMaxSize (const 12) $ do  -- limit combinatorial explosion
    prop "solutions have N queens in different rows" $ \(Positive n) ->
      classify (n < 4) "trivial" $
      queens n `shouldSatisfy` all (\l -> sort l == [1..n])

    prop "does not have queens attacking each other" $ \(Positive n) ->
      classify (n < 4) "trivial" $
